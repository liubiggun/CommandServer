...
0x66  0xaa     0x07      0x7f     "65536;0"      0x##      0xfc      控制获取图像数据，有两种类型的命令数据，分号区分参数。
...
第二种是修改图像传输模式："   30;1"第一个参数是帧数，长度不足5补空格，第二个参数是获取模式，0是获取双目数据，1是左边摄像头，2则右边。
...
class Command:
    ...
    def GetType(self,line):
        ...
        if self.typeNum == '\x7f':#验证图像命令数据的格式
            if (1024 < int(self.dataParts[0].strip()) < 65535) and self.dataParts[1] in ['^','&','*','$','?']:
                self.dataParts[0] = self.dataParts[0].strip()
                return True
            elif (0 < int(self.dataParts[0].strip()) < 31) and self.dataParts[1] in ['0','1','2']:#验证修改传图模式命令
                self.dataParts[0] = self.dataParts[0].strip()                                     #帧数[0,30]
                return True
            else:
                self.typeNum = '\x00'
                return False
